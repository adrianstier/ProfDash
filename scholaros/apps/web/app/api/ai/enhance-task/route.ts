...
export async function POST(request: Request) {
  try {
    ...
    const rateLimitResult = checkRateLimit(`ai:enhance:${rateLimitId}`, RATE_LIMIT_CONFIGS.ai);
...
Return ONLY valid JSON, no markdown or explanation.`;
    ...
    try {
      ...
      console.error("Failed to parse AI response for enhance-task:", textContent.text.substring(0, 500));
      return NextResponse.json(
